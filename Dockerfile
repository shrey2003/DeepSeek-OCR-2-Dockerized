...
# Copy model weights into the image (removes need for external volume)
COPY models/ /app/models/

# Copy the DeepSeek-OCR vLLM implementation
COPY DeepSeek-OCR-2/DeepSeek-OCR2-master/DeepSeek-OCR2-vllm/ ./DeepSeek-OCR2-vllm/

# Copy custom files to replace the originals (transparent replacement approach)
COPY custom_config.py ./DeepSeek-OCR2-vllm/config.py
COPY custom_image_process_ocr2.py ./DeepSeek-OCR2-vllm/process/image_process.py

# Copy custom run scripts to replace the originals
COPY custom_run_dpsk_ocr_pdf.py ./DeepSeek-OCR2-vllm/run_dpsk_ocr2_pdf.py
COPY custom_run_dpsk_ocr_image.py ./DeepSeek-OCR2-vllm/run_dpsk_ocr2_image.py
COPY custom_run_dpsk_ocr_eval_batch.py ./DeepSeek-OCR2-vllm/run_dpsk_ocr2_eval_batch.py

# Copy the startup script
COPY start_server.py .

# Copy requirements file and install additional dependencies
COPY DeepSeek-OCR-2/requirements.txt .

# Install Python dependencies from requirements and additional ones
...
ENV PYTHONPATH="/app/DeepSeek-OCR2-vllm:${PYTHONPATH}"
